...
parseFile :: FilePath -> IO (Either String [LexResult Broad.Payload])
parseFile fp = parse fp <$> T.readFile fp

parseStdin :: IO (Either String [LexResult Broad.Payload])
parseStdin = parse "<stdin>" <$> T.getContents

-- FIXME parse :: FilePath -> Text -> IO [Broad.Result]
parse :: FilePath -> Text -> Either String [LexResult Broad.Payload]
